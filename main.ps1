...
# Función para enviar mensaje al webhook principal al iniciar Windows
function SendMessageOnStartup {
    try {
        Write-Host "Enviando mensaje al webhook principal..."
        $message = "El PC está encendido: $($env:COMPUTERNAME)"
        Invoke-RestMethod -Uri $mainWebhookUrl -Method Post -Body @{message=$message} -ContentType 'application/json'
    } catch {
        Write-Host "Error al enviar el mensaje al webhook principal: $_"
    }
}

# Enviar mensaje al webhook principal al iniciar Windows
SendMessageOnStartup
